...
>
    ${essayDisplayHTML || '<i>Không có nội dung hướng dẫn.</i>'}
        ...
        div.innerHTML = `<div class="accordion-header" onclick="toggleAccordion(${teacherSafeJsArg(uniqueId)}, this)">
...
async function loadMaterialsListTeacher() {
    ...
    [...materials].reverse().forEach(mat => {
        ...
        div.innerHTML = `
            <div class="accordion-header" onclick="toggleAccordion(${teacherSafeJsArg(uniqueId)}, this)">
...
function teacherSafeHtml(value) {
    return escapeHTMLForMath(value);
}

function teacherSafeJsArg(value) {
    // JSON tạo literal JS đúng ngữ cảnh; escape HTML ngăn phá vỡ thuộc tính onclick.
    return teacherSafeHtml(JSON.stringify(String(value ?? '')));
}
...
async function loadSubmissions(isLoadMore = false) {
    ...
    submissions.forEach(sub => {
        ...
        if (hasTeacherPardonableViolation(sub)) {
            pardonHTML = `<button class="btn-approve" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; margin-left: 5px; border: 2px solid #059669;" onclick="pardonSubmission(${teacherSafeJsArg(sub._fbKey)})">✨ Tha lỗi (Xóa vi phạm)</button>`;
        }
        ...
        if (sub.isRedoing) {
            ...
            if (now > endTime) {
                actionHTML = `<button class="btn-reject" style="width: 100%; padding: 10px;" onclick="forceSubmitRedo(${teacherSafeJsArg(sub._fbKey)})">🔒 Khóa bài (Thu bài ngay)</button>`;
            } else {
            ...
            actionHTML = `<input type="number" id="grade-${sub.id}" placeholder="Điểm" max="10" min="0" style="margin: 0; width: 90px; text-align: center; font-weight: bold;" value="${hasGrade ? sub.grade : ''}">
                          <button class="btn-approve" onclick="gradeSubmission(${teacherSafeJsArg(sub.id)})">Lưu điểm</button>
                          <button class="btn-reject" onclick="requestRedo(${teacherSafeJsArg(sub._fbKey)})">Cho làm lại</button>`;

            if (hasGrade && !sub.isRegrading) {
                actionHTML += `<button class="btn-reject" style="background: linear-gradient(135deg, #74b9ff 0%, #0984e3 100%); color: white; margin-left: 5px;" onclick="requestRegrade(${teacherSafeJsArg(sub._fbKey)})">Chấm lại</button>`;
            }
            actionHTML += pardonHTML;
        }
...
">
    ⚠️ Học sinh chưa nộp tệp bài làm.
        ...
        div.innerHTML = `<div class="accordion-header" onclick="toggleAccordion(${teacherSafeJsArg(uniqueId)}, this)">
    <div class="accordion-title"><h4>${teacherSafeHtml(assign.title)}</h4><span>HS: <strong>${teacherSafeHtml(sub.studentName)}</strong> ${lateSubmissionBadge} ${missingEssayBadge}</span></div>
    <div class="accordion-meta"><span>${gradeStatus}</span><span class="toggle-icon">▼</span></div>
</div>
            <div id="${uniqueId}" class="accordion-content">${violationHTML}<span style="color: #888; font-size: 0.85em; display: block; margin-bottom: 10px;">🕒 Lần nộp cuối: ${teacherSafeHtml(sub.submitTime || 'Chưa rõ')}</span>
    ${watchStatusHTML}
    ${videoHTML}
                <div style="
    ...
    ${missingRequiredFileHTML}
                ...
                <div style="background: rgba(255,255,255,0.6); padding: 15px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.9);">
                    ...
                    <div style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed rgba(0,0,0,0.1);">
                        ...
                        <input type="file" id="teacherFile-${sub.id}" accept=".docx, .pdf, image/*" multiple onchange="handleTeacherFileAccumulate(this, ${teacherSafeJsArg(sub.id)})" style="padding: 10px; width: 100%; background: rgba(255,255,255,0.5);">
                        ...
                        <textarea id="teacherComment-${teacherSafeHtml(sub.id)}" placeholder="Nhập lời nhận xét cho học sinh..." rows="3" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid rgba(0,0,0,0.1); background: rgba(255,255,255,0.5);">${teacherSafeHtml(sub.teacherComment || '')}</textarea>
                    </div>` : ''}
                </div>
            </div>`;
